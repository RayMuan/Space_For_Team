...
//dto.getS_num() , dto.getUser_num() 다른 곳에서 가져오는 작업 아직 안 햇음
...
//			dto.setBk_endtime(bk_endtime);
			...
			String sql="insert into booking(s_num,user_num,bk_usercount,bk_date,bk_price,bk_usedate,bk_starttime,bk_endtime) values(?,?,?,?,?,?,?,?)";
...
//			num을 기준으로 오름차순인 것을  => 최근글이 위로 올라오도록 내림차순으로 변경
			String sql = "select * from booking order by num desc limit ?, ?";
			pstmt=con.prepareStatement(sql);
